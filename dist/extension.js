...
const readRulesContent = async (workspacePath, configuredPath) => {
    const normalizedPath = path.isAbsolute(configuredPath)
        ? configuredPath
        : path.join(workspacePath, configuredPath);
    try {
        const data = await fs_1.promises.readFile(normalizedPath, 'utf8');
...
function activate(context) {
    const reviewer = async (request, chatContext, stream, token) => {
        ...
        const rulesFile = (config.get('rulesFile') || '').trim();
        let diffChunk;
        try {
        ...
        sections.push('输出格式：', '```json', '{', '  "comments": {', '    "<file path>": [', '      { "line": "<line context>", "message": "[<Severity>] <description>" }', '    ]', '  }', '}', '```', '没有问题时请返回 `{"comments": {}}`，重点关注潜在缺陷、风险及遗漏的测试。');
